...
export default function PerformanceAnalytics() {
  ...
  const totalHoursPerSubject = data.reduce(
    (acc, session) => {
  ...
  const averagePerformance = data.reduce(
    (acc, session) => {
  ...
  const subjectPerformance = Object.entries(averagePerformance).map(
    ([subject, scores]) => {
  ...
  const barChartData = Object.entries(totalHoursPerSubject).map(
    ([subject, hours]) => ({
      subject,
      hours,
    }),
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10  mt-10">
      <div className="flex flex items-center justify-between items-center mb-6">
        ...
        <Button
          onClick={() => router.push("/dashboard/performance-analytics/create")}
        >
          + Add Session
        </Button>
      </div>
      ...
      <div>
        ...
        <ul>
          {data.map((session) => (
            ...
            >
              <div>
                <strong>{session.subject}</strong> - {session.hours} hrs,
                Performance: {session.performance ?? "N/A"}
              </div>
              <div className="flex gap-2">
                <Button
                  ...
                  onClick={() =>
                    router.push(
                      `/dashboard/performance-analytics/edit/${session._id}`,
                    )
                  }
                >
                  Update
